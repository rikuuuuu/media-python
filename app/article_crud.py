...
def create_article(db: Session, article: schemas.ArticleCreate, userID: int):
    db_item = models.Article(title=article.title, description=article.description, thumbnailURL=article.thumbnailURL, owner_id=userID)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
...
def update_article(db: Session, article: schemas.ArticleUpdate):
    up_article = db.query(models.Article).filter(models.Article.id == int(article.id)).first()
...
def image_upload(file: UploadFile = File(...)):
    ...
    try:
        s3.upload_fileobj(file.file, getenv('S3_BUCKET_NAME'), upload_filename, ExtraArgs={"ContentType": "image/jpeg", 'ACL':'public-read'})
